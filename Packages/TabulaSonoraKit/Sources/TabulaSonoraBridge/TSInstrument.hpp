...
private:
    ...
    /// The generator's event staging, in engine frames, cached so `latency_seconds` can answer
    /// without taking the lock: a host asks for the latency from whichever thread it likes.
